 ...
 if (!pagerank.background) {
     pagerank.popup = {
         ...
         trim: function(str, chars) {
             return pagerank.popup.ltrim(pagerank.popup.rtrim(str, chars), chars);
         },
         ...
         hexEncodeU32: function(b) {
             var c = pagerank.popup.toHex8(b >>> 24);
             c += pagerank.popup.toHex8(b >>> 16 & 255);
             c += pagerank.popup.toHex8(b >>> 8 & 255);
             return c + pagerank.popup.toHex8(b & 255);
         },
         ...
         awesomeHash: function(b) {
             ...
             return pagerank.popup.hexEncodeU32(c);
         },

         getPageRank: function(url) {
             var hash = pagerank.popup.awesomeHash(url.split("#")[0].split("//")[1]);
         ...
         getAlexaRank: function(domainString) {
             var alexaUrl = "http://xml.alexa.com/data?cli=10&dat=nsa&ver=quirk-searchstatus&url=" + domainString;
             var httpRequestAlexa = new XMLHttpRequest();
             httpRequestAlexa.open("GET", alexaUrl, true);
             httpRequestAlexa.onreadystatechange = function() {
                 if (httpRequestAlexa.readyState == 4) {
                     if (httpRequestAlexa.status == 200) {
                         var output = httpRequestAlexa.responseText;
                         var valueRank = output.split("<POPULARITY")[1].split('"/>')[0].split('TEXT="')[1].split('" SOURCE=')[0];
                         console.log('Alexa-label-' + valueRank);
                         var alexaLabel = document.getElementById('alexa-value');
                         alexaLabel.innerHTML = valueRank;
                     }
                 }
             };
             httpRequestAlexa.send();
         },
         getCurrentTabCallback: function(tab) {
             var url = tab.url;
             var domainString = url.split('/')[2];
             var pr = pagerank.popup.getPageRank(url);
             var alexa = pagerank.popup.getAlexaRank(domainString);

             //show alexa traffic graph
             var alexaGraph = document.getElementById("alexa-graph");
             var domain = pagerank.popup.getDomain(url, true);
             var host = pagerank.popup.getDomain(url, false);
             console.log(domain);
             alexaGraph.src = "http://traffic.alexa.com/graph?w=400&h=220&o=flt&c=1&y=t&b=ffffff&n=666666&r=2y&f=999999&u=" + domain
             pagerank.tracking.track("PopupOpened", {
                 domain: host
             });

         },
         getCurrentWindowCallback: function(window) {
             chrome.tabs.getSelected(window.id, pagerank.popup.getCurrentTabCallback)
         },
         onLoad: function() {
            chrome.windows.getCurrent(pagerank.popup.getCurrentWindowCallback);
         }
     };
 }
 window.addEventListener('load', function() {
     pagerank.popup.onLoad();
 }, false);
